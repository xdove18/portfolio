...
export default function Header() {
  ...
  /* Раньше здесь запрещалась прокрутка страницы, пока открыто
     меню. Теперь это не нужно и даже вредно.

     Не нужно — потому что выпадающее меню прикреплено к самой
     шапке и едет вместе с ней: фон под ним больше никуда
     не «уезжает».

     Вредно — потому что запрет прокрутки отключает прилипание
     шапки. Стоило один раз открыть меню и что-то пошло не так
     с отменой запрета — и шапка переставала держаться наверху
     до конца визита. Именно из-за этого она «не закреплялась»
     на телефоне. */
